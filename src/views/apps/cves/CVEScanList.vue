        ...
        <div class="card-body pt-0 overflow-scroll h-100 p-0 m-0 ">
            <KTDatatable @on-sort="sort" @on-items-select="onItemSelect" :data="list" :header="headerConfig"
                ...
                <template v-slot:actions="{ row: customer }">
                    <router-link :to="`/target-scan-detail/${CVEId}/${customer.id}/detail`" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
...
export default defineComponent({
    ...
    setup() {
        ...
        const getData = () => {
            ...
            return ApiService.get(`/cve/${getIdFromUrl()}/scan?search_scan=${query.value}&search_status=${filterStatus.value}&page=${currentPage.value}&page_size=${itemsPerPage.value}&ordering=${orderingID.value}`)
                .then(({ data }) => {
